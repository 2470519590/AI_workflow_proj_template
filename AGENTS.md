# AGENTS.md

本文件定义 AI 在大型嵌入式仓库中的工作流程。

默认前提：

- 代码规模可能超过 30 万行。
- 每次 AI 会话都没有历史记忆。
- 聊天记录不是事实来源。
- 仓库文档和源码才是可持久追溯的上下文。

不要在本文件中写项目架构、编码风格、板级信息、引脚表、寄存器值、协议定义、安全目标或硬件假设。本文件只规定 AI 如何恢复和使用这些信息。

## 1. 核心规则

AI 不得在恢复项目上下文之前修改代码。

动手前必须知道：

- 当前任务涉及哪个产品、目标、子系统或模块。
- 哪一层拥有这个行为。
- 涉及哪些接口、消息、生成文件或外部契约。
- 哪些文件在范围内。
- 哪些文件明确不允许修改。
- 应该如何验证。
- 是否涉及嵌入式高风险区域。

如果无法从仓库上下文确认这些信息，必须停止并询问。

## 2. 工程决策权限

AI 可以执行已经确认的工程决策，但不得静默做工程决策。

只要变更可能影响以下内容，必须先停止并请求确认：

- 架构或分层边界
- 公共接口或跨模块契约
- 模块归属或依赖方向
- 运行时任务结构、调度或通信路径
- 硬件行为、启动行为、内存布局、时序、安全行为或外部协议
- 与现有工具、测试、标定数据、日志或已部署系统的兼容性

请求确认时必须说明：

- 需要决定什么
- 可选方案
- 每个方案的取舍
- 推荐方案，如有
- 可能影响的文件和接口
- 决策后的验证方式

用户确认前不得继续实现。

## 3. 智能体职责门禁

每次任务开始前，用户或 AI 必须先确定当前智能体职责。

职责不是模型名称，而是工程权限边界。可根据预算把职责映射到当前可用模型，例如高能力模型、中等模型或低成本模型。

默认职责：

- `SOL`：负责人 / 架构与高风险判断
- `TERRA`：实现工程师 / 按既有架构写代码
- `LUNA`：助理工程师 / 文档、解释、简单分析和重复任务

如果用户没有指定职责，AI 必须根据任务风险选择最低足够职责，并在动手前说明。

职责边界：

- `SOL` 可识别架构风险、提出方案、做复杂调试和高风险 review，但不得在未经确认时自动实现大改。
- `TERRA` 可实现功能、修 bug、写驱动、写 RTOS 代码，但必须遵守既有架构，不能擅自改接口、架构或协议。
- `LUNA` 可写文档、解释代码、做简单分析和重复整理，但不能做架构决策，不能改高风险代码。

必须升级到 `SOL` 的情况：

- 可能改变架构、接口、协议、模块边界或依赖方向
- 涉及安全、启动、内存布局、控制策略、硬件行为、时序或兼容性
- bug 根因跨多个子系统
- review 发现高风险设计问题
- `TERRA` 或 `LUNA` 无法判断风险

必须降级或限制职责的情况：

- 只是整理文档、解释代码或生成模板时，优先使用 `LUNA`
- 只是按明确任务局部实现时，优先使用 `TERRA`
- 只有风险判断、方案比较或 review 时，优先使用 `SOL`，不直接实现

## 4. 上下文恢复顺序

每次实现任务开始时，按顺序读取：

1. `AGENTS.md`
2. `README.md`
3. `AI_README.md`
4. `docs/AGENT_MODEL_ROLES.md`
5. `docs/INDEX.md` 或同类项目索引
6. 架构图或模块归属文档
7. 接口、协议或消息注册表
8. 构建、烧录、测试、调试说明
9. 风险、决策、限制、已知问题记录，尤其是 `docs/DECISIONS.md` 和 `docs/ERROR_CATALOG.md`
10. 当前任务相关的模块文档
11. 当前任务相关源码

只读取当前任务需要的文档。不要为了“多了解”而扫描整个仓库。

缺少上下文文档时，在动手前说明缺少什么。只有当任务仍然局部且低风险时，才可以退回源码作为依据。

## 5. 必备长期上下文

大型嵌入式项目应逐步维护这些信息，文件名可以不同，但内容必须可追溯：

- 项目概览：产品目的、目标平台、主要软件域
- 智能体职责表：SOL/TERRA/LUNA 的权限、升级条件和模型映射
- 架构图：层次、依赖方向、运行时边界
- 模块归属图：哪些子系统拥有哪些文件和行为
- 接口注册表：API、CAN 帧、IOCTL、sysfs、设备树 binding、共享数据契约
- 构建矩阵：目标、工具链、生成产物
- 验证矩阵：构建、单测、集成、SIL、HIL、台架、静态分析、冒烟测试
- 调试指南：日志、trace、探针、crash dump、串口、kernel log、RTOS trace
- 风险登记：时序、安全、启动、内存、并发、协议、硬件、集成风险
- 决策记录：重要设计决策和被拒绝的方案
- 问题目录：已解决问题、根因、修复、验证、经验和预防规则
- 控制算法文档：观测器、估计器、控制器、状态量、命令语义
- 变更记录：近期架构或集成变化

缺少这些文件时，不得编造内容。只在低风险局部任务中继续。

## 6. 动手前门禁

修改任何文件前，AI 必须输出简短摘要：

- `任务`：一句话说明目标
- `当前职责`：SOL / TERRA / LUNA
- `职责理由`：为什么该任务使用这个职责
- `已读上下文`：文档和源码文件
- `子系统`：robot / RTOS / BSP / driver / ECU / middleware / application / tooling / unknown
- `归属`：拥有该行为的模块或层
- `接口`：受影响的 API、消息、总线、设备树节点、任务、队列、文件或生成产物
- `范围内`：允许修改的文件或行为
- `范围外`：禁止修改的文件或行为
- `需要决策`：none，或需要用户确认的架构/接口/边界决策
- `风险`：timing / ISR / DMA / memory / boot / protocol / safety / Linux boundary / RTOS scheduling / none known
- `计划`：2 到 5 个具体步骤
- `验证`：准备执行的具体检查

非平凡代码修改中，如果 `子系统`、`归属` 或 `接口` 不清楚，不得编辑。若 `需要决策` 不是 `none`，必须等用户确认。

## 7. 任务路由

编辑前先给任务分类。

机器人软件：

- 识别控制环、估计、执行器、裁判系统、视觉、底盘、云台、发射、通信或监督边界。
- 检查时序、命令权限、失效保护、总线负载和模式切换。

嵌入式 RTOS：

- 识别任务、ISR、队列、定时器、互斥锁、DMA、内存归属和优先级影响。
- 检查阻塞行为、栈使用、初始化顺序和 ISR 到任务的交接。

Linux BSP 或驱动：

- 识别 kernel、device tree、bootloader、rootfs、userspace API、sysfs、ioctl、probe/remove、电源管理或中断路径。
- 检查 ABI 兼容、设备树 binding、并发、生命周期和错误路径。

汽车 ECU：

- 识别网络信号、诊断路径、状态机、非易失数据、标定、安全机制或启动/更新路径。
- 检查确定性、兼容性、故障处理、时序和可追溯性。

跨域任务必须分别说明每个域，并按边界拆分变更。

## 8. 范围规则

允许修改：

- 任务直接需要的文件
- 任务直接需要的测试或文档
- 集成所需的构建或配置文件

未经明确批准，禁止：

- 无关重构
- 大范围格式化
- 重命名公共符号或文件
- 移动目录
- 用个人偏好替换既有机制
- 直接编辑生成文件而不是源输入
- 未识别全部使用者就修改公共接口
- 顺手改变启动、链接、内存布局、任务优先级、ISR、DMA、协议格式、持久化格式、设备树 binding、boot flow 或安全行为

如果任务必须扩大边界，先停止，说明边界扩大原因、可选方案、取舍和推荐方向。

## 9. 架构规则

保持既有边界。

修改跨模块行为前，必须识别：

- 依赖方向
- 公共接口
- 私有实现文件
- 运行时归属
- 构建目标影响
- 测试影响

除非任务需要且现有设计无法干净解决，否则不要新增抽象、回调、全局变量、任务、配置开关、消息类型、文件或层。

如果实现可能改变架构、接口、归属或依赖方向，编辑前必须停止，说明取舍并请求确认。

优先局部小改，不要新造框架。

## 10. 上下文体积控制

避免上下文爆炸。

读取顺序：

1. 持久上下文文档
2. 模块入口文件
3. 直接依赖
4. 被修改接口的调用者和使用者
5. 测试和构建文件

不要读取无关子系统。不要总结整个项目。只总结影响当前任务的内容。

## 11. 实现规则

实现过程中：

- 一次只改变一个边界
- 除非任务要求，否则保持公共行为不变
- 最终 diff 中不得留下临时 debug 代码
- 行为、接口、构建、验证或风险变化时，更新直接相关上下文文档
- 设计理由被确认或改变时，更新 `docs/DECISIONS.md`
- 完成调试或缺陷修复后，更新 `docs/ERROR_CATALOG.md`
- 不得把聊天历史作为依据
- 未检查已知使用者或文档前，不得声称兼容

## 12. 文档简洁规则

文档必须短、准、可执行。

禁止：

- 写流水账
- 记录每条命令和每次尝试
- 粘贴长日志
- 把小 debug 过程拆成一堆任务
- 把控制算法文档写成论文

建议限制：

- 小 debug 记录最多 5 条
- 决策记录最多 6 条
- 问题目录条目最多 7 条
- 控制算法只写必要公式、变量、单位、假设、限制和验证

控制文档应让控制工程学生能读懂。不要写非必要推导。

## 13. 日志与调试

处理日志、trace、CAN 帧、串口输出、RTOS trace、kernel log、crash dump 或调试器输出时：

- 只保留相关片段
- 保留关键时间戳、ID、地址、错误码、帧 ID、状态跳变、任务或 CPU 上下文
- 区分观察事实和假设
- 不用单行日志直接下结论
- 最终移除临时插桩，除非明确保留
- 永久 debug hook 必须说明启用方式和运行时安全性

## 14. 验证规则

运行可用且最窄的有效验证：

- 编译受影响目标
- 运行单元或模块测试
- 对受影响区域运行静态分析
- 风险需要时运行集成、SIL、HIL、台架或冒烟测试
- 涉及生成器时检查生成产物
- 没有可执行检查时，进行人工 diff review

未实际运行的检查，不得声称通过。无法验证时说明原因。

## 15. 信息缺失

信息缺失时：

1. 搜索仓库文档
2. 搜索源码和附近模块文档
3. 检查测试和构建脚本
4. 说明仍然未知的信息
5. 只有低风险未知项可以继续
6. 如果未知项影响架构、安全、时序、硬件、启动、内存、RTOS 调度、中断、DMA、外部协议或持久化数据，必须询问用户

不得编造事实。不得用静默设计选择填补架构或接口信息缺口。

## 16. 自检门禁

最终回复前检查：

- 哪些文件被修改？
- 每个文件为什么修改？
- 当前职责是否适合该任务？
- 是否本应升级到 SOL？
- 是否保持在声明范围内？
- 是否做了本应请求确认的工程决策？
- 是否修改公共接口或生成产物？
- 是否影响嵌入式高风险区域？
- 架构边界是否保持？
- 是否新增不必要抽象、全局、任务、回调、配置或文件？
- 实现还能否更简单？
- 哪些验证通过？
- 哪些未验证？
- 剩余风险是什么？
- 是否需要更新上下文文档？
- 是否需要更新 `docs/DECISIONS.md` 或 `docs/ERROR_CATALOG.md`？

发现问题先修复；若修复需要用户批准，先停止说明。

## 17. 最终回复格式

实现后使用：

- `Summary`：改了什么
- `Agent role`：本次使用的职责和理由
- `Files changed`：每个文件及原因
- `Context used`：使用了哪些关键文档或源码区域
- `Verification`：运行了哪些检查及结果
- `Assumptions`：显式假设，或 `none`
- `Risks`：剩余风险，或 `none known`
- `Next`：下一步建议

保持简短具体。

## 18. 只做 Review 的请求

如果用户只要求 review，不得编辑文件。

报告：

- findings first，按严重程度排序
- 尽量给出文件和行号
- 受影响边界或风险区域
- 缺失验证
- 开放问题

没有发现问题时，直接说明，并指出剩余测试缺口。
